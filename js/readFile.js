...
var requestProjetos = function() {
  var xhs = new XMLHttpRequest();
  xhs.open('GET', "https://api.github.com/gists/90a6307ce8f7685ee29238641fcbffb0", false);
  xhs.onreadystatechange = function() {
    if(xhs.readyState === 4) {
      if(xhs.status === 200 || xhs.status == 0) {
        var file = xhs.responseText;
        projetos = JSON.parse(file);
        projetos = JSON.parse(projetos.files.projetos.content);
      }
    }
  }
  xhs.send(null);
